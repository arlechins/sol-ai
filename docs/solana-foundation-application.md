...
| SDK | `@taopp/solana` — `attest()`, `challenge()`, `getScore()`, `discover()`; 19 tests, including a live-validator end-to-end suite and typed program errors |
...
| CI | Builds, lints, tests, deploys to a local validator, and audits dependencies on every push |
...
| Already in repo | All instructions implemented; 59 Rust tests; devnet deployment + verified loop; security hardening (vault donation sweep, substituted-account and re-init tests) |
| Funded remaining work | Mainnet deployment (program rent ~2.4 SOL + operator fees), reproducible/verifiable build (`solana-verify`) and published build hash, external code review of bond custody paths, operator runbooks (multisig upgrade authority), post-deploy verification on both clusters |
...
| Already in repo | SDK, MCP server 0.2, example agent; 19 SDK tests + 3 MCP tests; packaging validated with `publint` and `arethetypeswrong`; example verified on devnet |
