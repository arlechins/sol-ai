...
| Program tests | 94 Rust tests including a dedicated attack suite (donations, substituted accounts, unauthorized privileged operations, re-init attempts) |
...
| Already in repo | All instructions implemented; 94 Rust tests; devnet deployment with a verified loop; reproducible build verified hash-for-hash against devnet; security hardening (vault donation sweep, substituted-account and re-init tests) |
