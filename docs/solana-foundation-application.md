...
| SDK | `@taopp/solana` — `attest()`, `challenge()`, `getScore()`, `discover()`; 18 tests, including a live-validator end-to-end suite and typed program errors |
...
| Already in repo | SDK, MCP server 0.2, example agent; 18 SDK tests + 3 MCP tests; packaging validated with `publint` and `arethetypeswrong`; example verified on devnet |
