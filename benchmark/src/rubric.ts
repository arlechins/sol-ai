...
export function buildMechanismReport(
  ...
  const sybil = selected.has("sybil")
    ? runSybilFarming(mechanism, configs.sybil)
    : undefined;
  ...
  if (scores.sybilFarming !== undefined && scores.sybilFarming < 50) {
    weakSpots.push("Sybil farming is cheap (self-attestation has no verifier)");
  }
  ...
  if (scores.collusiveRing !== undefined && scores.collusiveRing < 50) {
    weakSpots.push("Fabricated peer ratings are accepted at face value");
  }
