...
function evaluate(
  ...
  const precision =
    flagged.size === 0 ? 0 : truePositives / flagged.size;
...
/**
 * Collusive mutual-rating ring: M accounts rate every other ring member, mixed
 * into a graph of honest, mostly non-reciprocating raters. A mechanism that
 * counts peer feedback lets the ring manufacture reputation with no external
 * counterparties; a mechanism that ignores peer feedback (TAOP v0.1) is
 * structurally immune but also has no interaction grounding.
 *
 * Detection is evaluated on the rating graph the mechanism actually records:
 * for mechanisms that ignore ratings there is nothing to inspect.
 */
export function runCollusiveRing(
  ...
  const findings =
    ringScorePerAgent === 0
      ? "Peer ratings do not contribute to the score, so the ring manufactures nothing and there is no " +
